 ...
 */
const COMPARISON = [
  { term: "A stairlift", detail: "Leaves the staircase in place and adds a visible aid to it." },
  { term: "A conventional homelift", detail: "Needs its own position in the house." },
  { term: "Elevante", detail: "Uses the space the staircase already occupies." },
];
      ...
      <Reveal className="container-content">
        <div className="grid items-end gap-10 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-4 lg:col-start-9">
            <div data-reveal-clip className="h-[70vw] max-h-[26rem] sm:h-[24rem]">
              <div data-parallax="0.22" className="h-full">
                <MediaFrame asset={content.media} sizes="(min-width: 1024px) 30vw, 90vw" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-8 border-t border-stone pt-8 lg:mt-14 lg:grid-cols-12 lg:gap-8">
          <p data-reveal className="max-w-[46ch] text-body-l text-charcoal-soft lg:col-span-5">
            {content.body}
          </p>
          <dl className="grid gap-x-8 gap-y-5 sm:grid-cols-3 lg:col-span-6 lg:col-start-7">
            {COMPARISON.map((row, i) => (
              <div key={row.term} data-reveal className="border-t border-stone pt-4">
                ...
                <dd className="mt-2 text-body text-charcoal-soft">{row.detail}</dd>
              </div>
            ))}
          </dl>
        </div>
