...
type Panel =
  { kind: "step"; step: Step } | { kind: "image"; media: MediaAsset };
    ...
    >
      ...
      >
        ...
        >
          <ol
            data-track
            className="flex flex-col gap-8 pr-gutter lg:w-max lg:flex-row lg:items-start lg:gap-10 lg:pr-32"
          >
            {items.map((item) =>
              item.kind === "step" ? (
                <li
                  key={item.step.number}
                  className="border-t border-stone pt-3 lg:w-[19rem] lg:shrink-0"
                >
                  <span
                    aria-hidden
                    className="block text-[clamp(2.75rem,5vw,4.25rem)] leading-none font-medium tracking-[-0.05em] text-stone"
                  >
                    {item.step.number}
                  </span>
                  ...
                  <p className="mt-2 max-w-[32ch] text-body text-charcoal-soft">
                    {item.step.body}
                  </p>
                </li>
              ) : (
                <li key={item.media.id} className="lg:shrink-0">
                  <div
                    data-reveal-clip
                    className="h-[58vw] max-h-[20rem] sm:h-[18rem] lg:h-[clamp(13rem,34svh,19rem)]"
                  >
                    <MediaFrame
                      asset={item.media}
