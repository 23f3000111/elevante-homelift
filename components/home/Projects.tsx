...
export function Projects({ content, projects }: ProjectsProps) {
  ...
  return (
    <section ref={ref} id="projects" aria-labelledby="projects-title" className="overflow-x-clip bg-white py-section-sm lg:py-0">
      <div data-stage className="lg:flex lg:h-[calc(100svh-4rem)] lg:flex-col lg:justify-center lg:py-6">
        ...
        >
          <ol data-track className="flex flex-col gap-12 pr-gutter lg:w-max lg:flex-row lg:items-start lg:gap-10 lg:pr-32">
            {projects.map((project, i) => (
              <li key={project.id} className="w-full shrink-0 lg:w-[26rem]">
                <Link href={content.cta.href} className="group block border-t border-charcoal pt-3">
                  <div className="flex items-baseline justify-between gap-8">
                    ...
                    <span className="font-mono text-small text-caption">
                      {project.placeholder ? content.locationPlaceholder : project.location}
                    </span>
                  </div>
                  <div data-reveal-clip className="mt-3 h-[58vw] max-h-[20rem] sm:h-[18rem] lg:h-[clamp(13rem,34svh,19rem)]">
                    <MediaFrame asset={project.media} tone="warm-white" sizes="(min-width: 1024px) 26rem, 92vw" imgClassName={IMG_HOVER} />
                  </div>
                  <h3 className="mt-4 text-[1.375rem] leading-tight font-medium text-charcoal">{project.title}</h3>
