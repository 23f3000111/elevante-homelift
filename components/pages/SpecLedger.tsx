...
export function SpecLedger({ title, note, rows }: { title: string; note: string; rows: SpecRow[] }) {
  return (
          ...
          <dl className="lg:col-span-7 lg:col-start-6">
            {rows.map((row, i) => (
              <div
                key={row.topic}
                data-reveal
                className={cn("grid gap-2 border-t py-5 sm:grid-cols-12 sm:gap-6", row.placeholder ? "border-dashed border-warm-grey" : "border-stone")}
              >
                <dt className="flex items-baseline gap-4 sm:col-span-5">
                  ...
                  <span className="text-body font-medium text-charcoal">{row.topic}</span>
                </dt>
                <dd className={cn("text-body sm:col-span-7", row.placeholder ? "font-mono text-small text-caption" : "text-charcoal-soft")}>{row.status}</dd>
              </div>
            ))}
          </dl>
        </div>
